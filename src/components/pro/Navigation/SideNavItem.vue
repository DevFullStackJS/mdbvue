<template>
  <component :is="tag" :class="liClass">
    <a v-if="href" :href="href" v-bind="$attrs" :class="className" @click="wave" tabindex="0">
      <mdb-icon v-if="icon" :class="iconClass" />
      <span v-if="slim" class="sv-slim">{{slim}}</span>
      <span v-if="slimIcon" class="sv-slim"><mdb-icon :icon="slimIcon" :far="slimFar" :fal="slimFal" :fad="slimFad" :fab="slimFab"/></span>
      <slot></slot>
    </a>
    <a v-if="toggler" v-bind="$attrs" :class="className" @click="handleClick" @keyup.enter="handleClick" tabindex="0">
      <mdb-icon v-if="!isCollapsed" :icon="icon" />
      <slot v-if="!isCollapsed"></slot>
      <mdb-icon :icon="openIcon" v-if="isCollapsed"/>
    </a>
    <router-link v-if="to" :to="to" v-bind="$attrs" :class="className" @click="wave" tabindex="0">
